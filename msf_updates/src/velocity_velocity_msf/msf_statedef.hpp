...
// States varying during propagation - must not change the ordering here for
    ...
    msf_core::StateVar_T<Eigen::Quaternion<double>, q_iv>, ///< Rotation from the IMU frame to the velocity sensor frame expressed in the IMU frame.
    msf_core::StateVar_T<Eigen::Matrix<double, 3, 1>, p_iv>  ///< Translation from the IMU frame to the velocity sensor frame expressed in the IMU frame.
> fullState_T;
}
