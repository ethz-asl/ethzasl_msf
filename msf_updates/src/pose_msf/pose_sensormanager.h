...
class PoseSensorManager : public msf_core::MSF_SensorManagerROS<
 ...
 private:
  ...
  bool InitTransform(sensor_fusion_comm::InitTransform::Request &req,
    ...
    Eigen::Matrix<double, 3, 1> p_wv(req.transform_wv.translation.x,
                                     req.transform_wv.translation.y,
                                     req.transform_wv.translation.z);
    Eigen::Quaternion<double> q_wv(req.transform_wv.rotation.w,
                                   req.transform_wv.rotation.x,
                                   req.transform_wv.rotation.y,
                                   req.transform_wv.rotation.z);
    ROS_INFO_STREAM(
        "Initialize filter with transform p_wv = [" << p_wv.transpose() << "], "
        "q_wv = ["<<STREAMQUAT(q_wv)<<"], scale "<<req.scale);
  ...
  void InitTransform(const Eigen::Matrix<double, 3, 1> &p_wv,
    ...
    Eigen::Quaternion<double> q, q_ic, q_vc;
